...
const initialFinalDocState: FinalDoc = {
  finalDocDetails: {
    title: "",
    description: "",
    dueDate: "",
...
export default function SupervisorSetDoc(_props: { onClose?: () => void }) {
  ...
  const handleChange = (
    ...
    setFinalDoc((prevFinalDoc) => ({
      ...prevFinalDoc,
      finalDocDetails: {
        ...prevFinalDoc.finalDocDetails,
        [name]: value,
      },
    }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Failed to update finalDoc");
      }

      const updatedFinalDoc = await response.json();
      setFinalDoc(updatedFinalDoc);
      alert("FinalDoc updated successfully");
      onClose();
    } catch (error) {
      console.error("Failed to update finalDoc:", error);
    }
  };
                      ...
                      <div className="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                        <div className="col-span-full mb-1">
                          ...
                          <input
                            ...
                            value={finalDoc?.finalDocDetails?.title}
                        ...
                        <div className="col-span-full mb-1">
                          ...
                          <input
                            ...
                            className="dui-input dui-input-bordered dui-input-primary w-full"
                            value={finalDoc?.finalDocDetails?.description}
                            onChange={handleChange}
                            required
                          />
                        </div>
                        ...
                        <div className="col-span-full mb-1">
                          <label
                            htmlFor="finalDocGuidelineFile"
                            className="block text-sm font-medium leading-6 text-gray-900 mb-2"
                          >
                            Upload Final Documentation Guideline File
                          </label>
                          <input
                            type="text"
                            name="finalDocGuidelineFile"
                            placeholder="Paste your link (Google Drive, OneDrive, etc.)"
                            className="dui-input dui-input-bordered dui-input-primary w-full"
                            value={
                              finalDoc?.finalDocDetails?.finalDocGuidelineFile
                            }
                            onChange={handleChange}
                            required
                          />
