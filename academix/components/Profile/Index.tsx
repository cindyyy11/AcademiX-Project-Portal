  ...
  return (
    <div className="">
      <div className="mb-8 lg:text-center md:mb-6">
        ...
        <div className="text-h4">{userData?.user?.name || "N/A"}</div>
        <div className="mb-4 text-sm">{userData?.user?.email || "N/A"}</div>
        <div className="mb-3 text-xs">
          Happiness is not something readymade. It comes from your own actions.
        </div>
        <div className="label-stroke min-w-[5.125rem]">
          {userData?.user?.role || "Designer"}
        </div>
      </div>
      <div>
            ...
            <div className="flex justify-between items-center">
              ...
              {item.counter && (
                <Icon
                  ...
                  name={item.counter > 0 ? "arrow-up-right" : "arrow-down-left"}
                />
              )}
            </div>
          </div>
